  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <button onClick={() => setFilterStatus('')} className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${filterStatus === '' ? 'bg-green-800 text-white border-green-800' : 'bg-white text-slate-600 border-slate-300'}`}>Todos</button>
        {Object.entries(statusLabels).map(([k, v]) => (
          <button key={k} onClick={() => setFilterStatus(k)} className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${filterStatus === k ? 'bg-green-800 text-white border-green-800' : 'bg-white text-slate-600 border-slate-300'}`}>{v}</button>
        ))}
      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input type="number" min={1} value={form.quantity} onChange={e => setForm({...form, quantity: parseFloat(e.target.value)})} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
                </div>
                <div>
